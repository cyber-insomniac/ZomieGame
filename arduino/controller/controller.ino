...
// Edge-detection state for the heal button so holding it doesn't spam 'g'
bool healButtonLastState = false;

void setup() {
    Serial.begin(9600);
    while (!Serial) {
    delay(10); // wait for native USB serial to actually connect
    };
    ...
    // If HEAL is a pushbutton wired to GND (no external resistor), use
    // INPUT_PULLUP and treat LOW as "pressed" - see handleHeal() below.
    // If you've wired an external pull-down and it reads HIGH when pressed,
    // switch back to plain INPUT and flip the logic in handleHeal().
...
void loop() {
    // These run every pass, independent of punch detection, so aiming
    // and healing are live all the time - not just at the instant of a hit.
    int buttonstate = digitalRead(HEAL);
    if (buttonstate == LOW {
...
void handleHeal() {
    ...
    // (button wired between the pin and GND).
